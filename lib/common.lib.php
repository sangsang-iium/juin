...
// }
	...
	if($email)
		$str2 .= "<a href=\"".BV_ADMIN_URL."/formmail.php?mb_id=".$mb_id."&name=".urlencode($name)."&email=".$email."\" onclick=\"win_open(this,'win_email','650','580','no'); return false;\">메일보내기</a>\n";

	if($phone)
		$str2.= "<a href=\"".BV_ADMIN_URL."/sms/sms_user.php?ph={$phone}\" onclick=\"win_open(this,'win_sms','300','360','no'); return false;\">SMS보내기</a>\n";
	...
	if(is_seller($mb_id))
		$str2.= "<a href=\"".BV_ADMIN_URL."/admin_ss_login.php?mb_id={$mb_id}&lg_type=S\" target=\"_blank\">공급사로그인</a>\n";
...
function display_logo($filed='basic_logo')
{
	...
	$file = BV_DATA_PATH.'/banner/'.$row[$filed];
	if(is_file($file) && $row[$filed]) {
		$file = rpc($file, BV_PATH, BV_URL);
		return '<a href="'.BV_URL.'"><img src="'.$file.'"></a>';
	} else {
		return '';
	}
}
...
function get_popular($rows, $pt_id)
{
	...
	for($i=0; $row=sql_fetch_array($result); $i++){
		...
		$href = BV_SHOP_URL.'/search.php?ss_tx='.$word;
...
function board_latest($boardid, $len, $rows, $pt_id)
{
	...
	for($i=0;$row=sql_fetch_array($res);$i++){
		...
		$href  = BV_BBS_URL."/read.php?boardid={$boardid}&index_no={$row['index_no']}";
...
function get_look_ahead($it_img, $it_img_del)
{
	...
	if(preg_match("/^(http[s]?:\/\/)/", $it_img) == true)
		...
		$file_url = BV_DATA_URL."/goods/".$it_img;
...
function get_admin($mb_id)
{
    ...
    if(is_admin())
		return BV_ADMIN_URL.'/';
    if(is_partner($mb_id))
		return BV_MYPAGE_URL.'/page.php?code=partner_info';
    if(is_seller($mb_id))
		return BV_MYPAGE_URL.'/page.php?code=seller_main';
...
function get_change_select($name, $selected='', $event='')
{
	...
	if(!in_array($selected, array(2,3,4,5,7,9,18))) {
...
function get_category_head_image($ca_id)
{
	...
	$file = BV_DATA_PATH.'/category/'.$row['headimg'];
...
class str_encrypt
{
    ...
    {
        if(!$salt)
            $this->salt = md5(preg_replace('/[^0-9A-Za-z]/', substr(BV_MYSQL_USER, -1), BV_MYSQL_PASSWORD));
        else
            $this->salt = $salt;
