...
if (in_array($_POST['paymethod'], array('무통장', '포인트'))) {
  ...
  if ($toss_acc->code) {
    if ($resulturl == 'pc') {
      ...
      $toss_error = "가상계좌 결제 오류 " . $t_amount . $t_orderid . $t_ordername . $t_name . $t_email . $t_bank . $customerMobilePhone;
    } else {
  ...
  if ($toss_run->code) {
    if ($resulturl == 'pc') {
      ...
      $toss_error = "결제 오류 " . $toss_run->code;
    } else {
...
if (in_array($_POST['paymethod'], array('무통장', '포인트'))) {
  if ($resulturl == 'pc') {
    $returnUrl = (BV_URL . '/mng/shop/orderinquiryview.php?od_id=' . $od_id . '&uid=' . $uid . '&tran_id=' . $tran_id . '&reg_yn=' . $reg_yn);
  } else {
    $returnUrl = (BV_MSHOP_URL . '/orderinquiryview.php?od_id=' . $od_id . '&uid=' . $uid . '&tran_id=' . $tran_id . '&reg_yn=' . $reg_yn);
  }
} else if ($_POST['paymethod'] == 'KAKAOPAY') {
  $returnUrl = (BV_MSHOP_URL . '/orderkakaopay.php?od_id=' . $od_id);
} else if ($_POST['paymethod'] == '삼성페이') {
  $returnUrl = (BV_MSHOP_URL . '/orderinicis.php?od_id=' . $od_id);
} else if ($_POST['paymethod'] == '신용카드') {
  if ($resulturl == 'pc') {
    $returnUrl = (BV_URL . '/mng/shop/orderinquiryview.php?od_id=' . $od_id . '&uid=' . $uid . '&tran_id=' . $tran_id . '&reg_yn=' . $reg_yn);
  } else {
  ...
  if ($default['de_pg_service'] == 'kcp') {
    $returnUrl = (BV_MSHOP_URL . '/orderkcp.php?od_id=' . $od_id);
  } else if ($default['de_pg_service'] == 'inicis') {
    $returnUrl = (BV_MSHOP_URL . '/orderinicis.php?od_id=' . $od_id);
  } else if ($default['de_pg_service'] == 'lg') {
    $returnUrl = (BV_MSHOP_URL . '/orderlg.php?od_id=' . $od_id);
  }
