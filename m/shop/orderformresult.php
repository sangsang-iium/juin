...
if(!$i_price) {
    alert("결제할 금액이 없습니다.", BV_MURL);
}

if(in_array($od_settle_case, array('무통장','포인트'))) {
    alert("올바른 방법으로 이용해 주십시오.", BV_MURL);
}
...
if($od_settle_case != 'KAKAOPAY') {
    if($default['de_pg_service'] == 'kcp' && ($_POST['tran_cd'] == '' || $_POST['enc_info'] == '' || $_POST['enc_data'] == ''))
        alert('결제등록 요청 후 주문해 주십시오.', BV_MSHOP_URL.'/orderkcp.php?od_id='.$od_id);

    if($default['de_pg_service'] == 'lg' && !$_POST['LGD_PAYKEY'])
        alert('결제등록 요청 후 주문해 주십시오.', BV_MSHOP_URL.'/orderlg.php?od_id='.$od_id);

    if($default['de_pg_service'] == 'inicis' && !$_POST['P_HASH'])
        alert('결제등록 요청 후 주문해 주십시오.', BV_MSHOP_URL.'/orderinicis.php?od_id='.$od_id);
}
...
if($od_settle_case == "계좌이체")
{
    switch($default['de_pg_service']) {
        case 'lg':
            ...
            break;
        case 'inicis':
            include BV_MSHOP_PATH.'/inicis/pay_result.php';
            break;
		case 'kcp':
            include BV_MSHOP_PATH.'/kcp/pp_ax_hub.php';
