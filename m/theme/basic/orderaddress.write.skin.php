...
<form name="b_saveform" id="b_saveform">
  ...
  <div id="sod_addr_write">
    <div class="form-wrap">
      ...
      <!-- * 배송요청사항 추가  _20240713_SY -->
      <div class="form-row">
        <div class="form-head">
          <p class="title">배송요청사항
          </p>
        </div>
        <div class="form-body address">
          <input type="text" name="b_addr_req" id="b_addr_req" value="<?= $b_addr_req ?>" class="w-per100 frm-input">
        </div>
        <div class="frm-choice set_df_addr_wrap">
          <input type="checkbox" name="b_base" id="set_df_addr" value="1" <?php if ($b_base == "1") echo "checked"; ?>>
          <label for="set_df_addr">기본배송지로 설정</label>
  ...
  $(".od-dtn__add").click(function() {
    ...
    $.ajax({
      ...
      data: {
        ...
        // 기본배송지 체크여부로 구분 되도록 수정 _20240713_SY
        'b_base': $("#set_df_addr").prop('checked') ? '1' : '0', 
        'b_wr_id': $("#b_wr_id").prop('value'),
        'b_addr_req': $("#b_addr_req").prop('value'),
      },
      dataType: "text",
