...
if(isset($_POST['email'])) {
    ...
    $frm_msg .= '<h5 class="htag_title marb20">결과 메세지</h5>';
    $frm_msg .= '<div class="local_desc01 local_desc"><p>';
    $frm_msg .= '다음 '.count($email).'개의 메일 주소로 테스트 메일 발송이 완료되었습니다.';
    $frm_msg .= '</p></div>';
    $frm_msg .= '<div class="text_box"><ul class="cnt_list step01">';
    for($i=0;$i<count($email);$i++) {
        $frm_msg .= '<li><b class="fc_084">'.$email[$i].'</b></li>';
    }
    $frm_msg .= '';
    $frm_msg .= '<li>해당 주소로 테스트 메일이 도착했는지 확인해 주십시오.</li>';
    $frm_msg .= '<li>만약, 테스트 메일이 오지 않는다면 더 다양한 계정의 메일 주소로 메일을 보내 보십시오.</li>';
    $frm_msg .= '<li>그래도 메일이 하나도 도착하지 않는다면 메일 서버(sendmail server)의 오류일 가능성이 높으니, 웹 서버관리자에게 문의하여 주십시오.</li>';
    $frm_msg .= '</ul></div>';
}
?>
...
<div class="text_box">
    <ul class="cnt_list step01">
        <li>사이트에서 메일을 발송했음에도 불구하고 메일을 수신 하지 못하는경우</li>
        <li>프로그램상의 문제이기 보다는 대부분 메일서버 혹은 받는 메일서버 중 문제가 발생했을 가능성이 있습니다.</li>
        <li>또는 메일이 정상적으로 발송이 되었으나 스팸처리로 이동되어 있을수도 있습니다.</li>
        <li class="fc_red">따라서 보다 정확한 테스트를 원하신다면 여러 곳으로 테스트 메일을 발송하시기 바랍니다.</li>
        <li>메일이 정상적으로 발송된 경우 아래내용과 같이 수신됩니다.</li>
        <li>[메일검사] 내용</li>
        <li>이 내용이 제대로 보인다면 보내는 메일 서버에는 이상이 없는것입니다.</li>
        <li><?php echo BV_TIME_YMDHIS; ?></li>
        <li>이 메일 주소로는 회신되지 않습니다.</li>
    </ul>
</div>
	...
	<tr>
		...
		<td>				
            ...
            <ul class="cnt_list step01 mart10">
                <li>여러곳으로 메일을 발송하시려면 콤마(,)로 메일주소를 구분 하십시오.</li>
                <li>여러곳으로 메일발송시 <em class="fc_084">예) abc@naver.com,abc@daum.net,abc@nate.com</em></li>
            </ul>
    ...
    <ul class="cnt_list step01">
        <li>E-mail 수신이 안되시나요?
            <ul class="cnt_list step02">
                <li>메일수신이 안되는경우 프로그램상에서 처리해드릴 수 있는 부분이 없습니다.</li>
                <li class="fc_red">호스팅회사나 서버관리자에게 메일발송테스트 한 결과를 설명하고 해결요청을 하시기 바랍니다.</li>
                <li>호스팅회사나 서버관리자가 화이트 도메인 등록이 필요하다고 요청한경우 <strong>한국정보보호진흥원 KISA</strong> <a href="http://www.kisarbl.or.kr" target="_blank">(http://www.kisarbl.or.kr)</a> 에서 등록합니다.</li>
                <li>등록후 SPF Record 레코드값을 호스팅 회사나 서버관리자에게 전달하고 적용을 요청합니다.</li>
            </ul>
        </li>
    </ul>
</div>
