...
<div class="new_win_body">
	...
	<div class="tbl_head01">
		...
		<tr class="<?php echo $bg; ?>">
			...
			<td><button type="button" onclick="yes('<?php echo $row['seller_code']; ?>', '<?php  echo $row['company_name'] ?>', '<?php echo $row['income_type']; ?>', '<?php echo $row['income_per_type']; ?>', '<?php echo $row['income_price']; ?>', '<?php echo $row['income_per']; ?>')" class="btn_small grey">선택</button></td>
		</tr>
		<?php
		}
		if($i==0)
...
<script>
  // name 추가_20240415_SY
function yes(mb_id, name='', type='', per_type='', price='', per=''){
  ...
  if(opener.document.fregform.in_type) {
    opener.document.fregform.in_type.value = type;
  }
  if(opener.document.fregform.in_per_type) {
    opener.document.fregform.in_per_type.value = per_type;
  }
  if(opener.document.fregform.in_price) {
    opener.document.fregform.in_price.value = price;
  }
  if(opener.document.fregform.in_per) {
    opener.document.fregform.in_per.value = per;
  }
	self.close();
}
</script>
